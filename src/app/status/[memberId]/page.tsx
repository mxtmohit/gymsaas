...
import { Gym, Plan, Member, Membership, Payment } from '@/lib/mockDb';
import {
  Calendar,
  Clock,
  CheckCircle,
...
export default function MemberStatusPage({ params }: { params: Promise<{ memberId: string }> }) {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        ...
        if (memberData) {
          setMember((prev) => {
            if (prev && prev.status !== memberData.status && memberData.status === 'active') {
  ...
  const handleSimulateApproval = async () => {
    ...
    try {
      ...
      const pendingPayment = gymPayments.find(p => p.member_id === member.id && p.status === 'pending');
  ...
  const expiryInfo = useMemo(() => {
    if (!member || !member.expires_at) return { daysLeft: 0, text: 'Expired', color: 'text-rose-600 bg-red-100 border-red-400' };

    const diffMs = new Date(member.expires_at).getTime() - Date.now();
    const daysLeft = Math.ceil(diffMs / (1000 * 60 * 60 * 24));

    if (daysLeft < 0) {
      return { daysLeft, text: 'Expired', color: 'text-red-650 bg-red-100 border-red-300' };
    } else if (daysLeft === 0) {
      return { daysLeft, text: 'Expires Today', color: 'text-amber-700 bg-amber-100 border-amber-300' };
    } else if (daysLeft === 1) {
      return { daysLeft, text: 'Expires Tomorrow', color: 'text-amber-700 bg-amber-100 border-amber-300' };
    } else {
      return { daysLeft, text: `${daysLeft} Days Remaining`, color: 'text-emerald-700 bg-green-100 border-green-300' };
    }
  }, [member]);
  ...
  const motivationalQuote = useMemo(() => {
    ...
    const sumCodes = memberId.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0);
  ...
  const handleRenewalSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === 'upi' && !utr && !screenshotUrl) {
      setSubmitError('Please provide the transaction reference ID or screenshot.');
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-brutal-bg flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-black font-extrabold text-sm uppercase">Loading Member Profile...</p>
        </div>
      </div>
    );
  }
  ...
  if (!member || !gym) {
    return (
      <div className="min-h-screen bg-brutal-bg flex flex-col justify-center items-center px-4 text-center">
        ...
        <h2 className="text-3xl font-extrabold text-black uppercase mb-2">Member Not Found</h2>
  ...
  const activePlanName = activeMembership 
    ? plans.find(p => p.id === activeMembership.plan_id)?.name || 'Standard Plan'
    : 'No Active Plan';
      ...
      <div className="max-w-xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center px-2">
          <div className="flex items-center gap-3">
            ...
            <span className="font-extrabold text-lg uppercase text-black">{gym.name}</span>
          </div>
          ...
          >
            <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''} stroke-[2.5]`} />
          </button>
        </div>
        ...
        <div className="neo-card p-8 bg-white border-3 border-black shadow-[6px_6px_0px_0px_#000000] text-center space-y-6 relative overflow-hidden">
          ...
          <div className="space-y-1">
            <h2 className="text-xs font-black uppercase tracking-wider text-slate-500">Membership Pass</h2>
            <h1 className="text-2xl font-extrabold uppercase text-black">{member.name}</h1>
            <p className="text-slate-800 text-xs font-mono font-bold">{member.phone}</p>
          </div>
          ...
          <div className="py-2.5 px-4 border-2 border-black rounded-lg bg-black text-white font-extrabold text-xs uppercase tracking-wide shadow-[2px_2px_0px_0px_#fdfe02]">
            {member.status === 'active' && "🎉 PASS ACTIVE! You are locked in. Time to crush your fitness goals! 💪"}
            {member.status === 'pending' && "⏳ HOLD TIGHT! Verification in progress. Get your gym gear ready to lift heavy! ⚡"}
            {member.status === 'expired' && "⚠️ PASS EXPIRED! Don't lose your gains. Power up and renew today! 🔥"}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-left font-bold">
            <div>
              <span className="block text-[10px] text-slate-550 uppercase font-extrabold">Active Subscription</span>
              <span className="block text-sm font-black text-black uppercase truncate mt-0.5">{activePlanName}</span>
            </div>
            <div className="text-right">
              <span className="block text-[10px] text-slate-550 uppercase font-extrabold">Valid Until</span>
              <span className="block text-sm font-black text-black font-mono mt-0.5">
                {member.expires_at ? new Date(member.expires_at).toLocaleDateString('en-IN', {
                  day: 'numeric',
                  month: 'short',
                  year: 'numeric'
                }) : 'N/A'}
          ...
          {/* Expiration badge */}
          <div className={`p-4 border-3 border-black rounded-xl flex items-center justify-between shadow-[2px_2px_0px_0px_#000000] ${expiryInfo.color}`}>
            <div className="flex items-center gap-3">
              <Clock className="w-5 h-5 text-black stroke-[2.5]" />
              <div className="text-left">
                <span className="block text-[9px] text-slate-700 uppercase font-black">Expiration Status</span>
                <span className="block text-xs font-black uppercase">{expiryInfo.text}</span>
              </div>
            </div>
          ...
          <div className="p-4 bg-[#f4f2ed] border-3 border-black rounded-xl shadow-[3px_3px_0px_0px_#000000] text-left">
            <span className="block text-[10px] text-slate-700 font-black uppercase tracking-wider mb-1">Encouragement Corner</span>
          ...
          {member.status === 'pending' && (
            <div className="bg-brutal-yellow/30 p-4 border-3 border-black rounded-xl space-y-3 shadow-[2px_2px_0px_0px_#000000]">
              <div className="flex items-start gap-3 text-left">
                ...
                <div>
                  <h4 className="text-xs font-black uppercase">Simulation Desk: Approve Payment</h4>
          ...
          {member.status === 'active' && expiryInfo.daysLeft <= 7 && (
            <div className={`p-4 rounded-xl border-3 border-black flex items-start gap-3 text-left shadow-[2px_2px_0px_0px_#000000] ${
              ...
              <div>
                <h4 className="text-xs font-black uppercase">
                  {expiryInfo.daysLeft <= 3 ? 'Membership Expiring Soon!' : 'Membership Expiry warning'}
                </h4>
                <p className="text-[10px] sm:text-xs font-bold leading-normal mt-1">
                  Your pass validity expires in {expiryInfo.daysLeft} days. Click below to renew subscription.
                </p>
              </div>
            </div>
          )}
          ...
          {member.status === 'expired' && (
            <div className="p-4 rounded-xl bg-brutal-red/25 border-3 border-black text-black flex items-start gap-3 text-left shadow-[2px_2px_0px_0px_#000000]">
              <AlertTriangle className="w-5 h-5 flex-shrink-0 mt-0.5 text-black stroke-[2.5]" />
              <div>
                <h4 className="text-xs font-black uppercase">Gym Access Blocked</h4>
                <p className="text-[10px] sm:text-xs font-bold leading-normal mt-1">
                  Your membership has expired. Please submit a payment proof below to unlock your pass.
                </p>
              </div>
            </div>
          )}
          ...
          {renewalSuccess && (
            <div className="p-4 rounded-xl bg-green-100 border-3 border-black text-black flex items-start gap-3 text-left shadow-[2px_2px_0px_0px_#000000]">
              ...
              <div>
                <h4 className="text-xs font-black uppercase">Renewal Proof Submitted</h4>
                <p className="text-[10px] sm:text-xs font-bold leading-normal mt-1">
        ...
        {showRenewal && (
          <div className="neo-card p-6 sm:p-8 bg-white border-3 border-black shadow-[6px_6px_0px_0px_#000000] space-y-6">
            <div className="flex justify-between items-center border-b-3 border-black pb-2">
              <h3 className="text-lg font-black uppercase text-black">Renew Membership</h3>
              <button 
            ...
            <form onSubmit={handleRenewalSubmit} className="space-y-6">
              ...
              {/* 1. Pick Plan */}
              <div>
                <label className="block text-xs font-black uppercase text-slate-800 tracking-wider mb-3">Choose Plan Option</label>
                  ...
                  {plans.map((p) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="font-extrabold uppercase text-black text-sm">{p.name}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-sm font-black text-black">₹{p.price}</span>
                        <span className="text-[9px] text-slate-750 font-mono font-bold ml-1">/{p.duration_days}d</span>
                      </div>
                    </label>
                  ))}
                </div>
                ...
                <div className="space-y-6 pt-4 border-t-3 border-black">
                  ...
                  {paymentMethod === 'upi' ? (
                    <>
                      <div className="bg-brutal-bg/40 p-5 rounded-xl border-3 border-black grid grid-cols-1 sm:grid-cols-2 gap-4 shadow-[2px_2px_0px_0px_#000000]">
                        ...
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <span className="text-[9px] font-bold text-slate-700 mt-1 uppercase">GPay / PhonePe Scanner</span>
                        </div>

                        {/* Pay summary */}
                        <div className="flex flex-col justify-between">
                          <div className="space-y-1">
                            <span className="text-[9px] text-slate-650 font-black uppercase">UPI Address</span>
                            <div className="text-brutal-orange font-bold font-mono text-xs select-all">{gym.upi_id || 'merchant@upi'}</div>
                          </div>
                          ...
                          <div className="flex justify-between items-baseline pt-3 border-t-2 border-black mt-3">
                            ...
                            <span className="text-lg font-black text-black">₹{selectedPlan.price}</span>
                          </div>
                        </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          <label className="block text-xs font-black uppercase text-slate-800 tracking-wider mb-2">Enter UPI Transaction UTR</label>
                          <input
                            type="text"
                        ...
                        <div>
                          ...
                          {screenshotUrl && (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={screenshotUrl} alt="Receipt preview" className="w-16 h-20 object-cover rounded mt-3 border-2 border-black bg-white" />
                          )}
                        </div>
                      </div>
                    </>
                    ...
                    <div className="p-5 bg-brutal-bg/40 border-3 border-black rounded-xl space-y-4 shadow-[2px_2px_0px_0px_#000000]">
                      <h4 className="text-xs font-black uppercase text-black">Cash Handover Instructions</h4>
                      <div className="text-slate-850 text-xs font-bold leading-relaxed space-y-1">
                        <p>1. Hand over the cash payment amount directly to the gym owner/trainer:</p>
                      </div>

                      <div className="bg-white p-3 border-3 border-black rounded-xl text-center shadow-[2px_2px_0px_0px_#000000]">
                        <div className="text-[9px] text-slate-500 font-black uppercase">Amount to pay in cash</div>
                        <div className="text-brutal-orange font-black text-xl mt-1">₹{selectedPlan.price.toLocaleString('en-IN')}</div>
                        <div className="text-[9px] text-slate-650 font-bold uppercase mt-1">Plan: {selectedPlan.name}</div>
                      </div>

                      <p className="text-[9px] text-slate-650 font-bold text-center leading-normal">
                        After paying at the counter, click the button below to submit. The owner will confirm and reactivate your pass immediately.
                      </p>
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className="w-full neo-btn neo-btn-orange text-white py-3.5 px-4 font-black uppercase shadow-[3px_3px_0px_0px_#000000]"
                  >
                    {submittingRenewal ? 'Submitting proof...' : (paymentMethod === 'upi' ? 'Submit Renewal Proof' : 'Request Cash Verification')}
                  </button>
