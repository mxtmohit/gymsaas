...
export default function Dashboard() {
    ...
    async function loadData() {
      try {
        ...
        setProfileForm({
          ...
          upi_qr_url: gymData.upi_qr_url || null,
          instagram_url: gymData.instagram_url || "",
          youtube_url: gymData.youtube_url || "",
          gallery_photo1: gymData.gallery_photo1 || null,
          gallery_photo2: gymData.gallery_photo2 || null,
          gallery_photo3: gymData.gallery_photo3 || null,
        });
        ...
        if (typeof window !== "undefined") {
          const msList = JSON.parse(
            window.localStorage.getItem("memberships") || "[]",
          );
          setMemberships(msList);
          const cleared = JSON.parse(
            window.localStorage.getItem("cleared_notifications") || "[]",
          );
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const updatedGym = {
        ...
        gallery_photo1: profileForm.gallery_photo1,
        gallery_photo2: profileForm.gallery_photo2,
        gallery_photo3: profileForm.gallery_photo3,
      };
  ...
  const handleDeleteMember = async (memberId) => {
    if (
      !confirm(
        "Are you sure you want to delete this member? All their payment and membership histories will be removed.",
      )
    )
      return;
    try {
  ...
  const handleSaveMemberExtension = async () => {
    ...
    try {
      ...
      const updatedMember = {
        ...
        status:
          new Date(extendExpiryDate).getTime() > Date.now()
            ? "active"
            : "expired",
      };
  ...
  const handleMarkExpired = async (member) => {
  ...
  const stats = useMemo(() => {
    ...
    const revenueThisMonth = payments
      .filter(
        (p) =>
          p.status === "approved" &&
          new Date(p.created_at).getTime() >= startOfMonth.getTime(),
      )
    ...
    const revenueLastMonth = payments
      .filter((p) => {
        ...
        return (
          payDate >= firstDayOfLastMonth && payDate < firstDayOfCurrentMonth
        );
      })
      .reduce((sum, p) => sum + p.amount, 0);
    ...
    const signupsThisMonth = members.filter((m) => {
      ...
      return (
        joinedDate.getMonth() === startOfMonth.getMonth() &&
        joinedDate.getFullYear() === startOfMonth.getFullYear()
      );
    }).length;
    const signupsLastMonth = members.filter((m) => {
      const joinedDate = new Date(m.joined_at);
      const prevMonth =
        startOfMonth.getMonth() === 0 ? 11 : startOfMonth.getMonth() - 1;
      const prevYear =
        startOfMonth.getMonth() === 0
          ? startOfMonth.getFullYear() - 1
          : startOfMonth.getFullYear();
      return (
        joinedDate.getMonth() === prevMonth &&
        joinedDate.getFullYear() === prevYear
      );
    }).length;
    ...
    const targetProgress = Math.min(
      Math.round((revenueThisMonth / targetRevenue) * 100),
      100,
    );
    ...
    const gymMemberships = membershipsList.filter((ms) =>
      gymMemberIds.has(ms.member_id),
    );
  ...
  const revenueChartData = useMemo(() => {
    ...
    return Object.entries(dataMap).map(([name, revenue]) => ({
      name,
      revenue,
    }));
  }, [payments]);
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-brutal-bg text-black font-sans">
      ...
      <header className="flex md:hidden sticky top-0 z-50 w-full h-16 bg-white border-b-3 border-black items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-black text-black text-xs uppercase truncate max-w-[140px] tracking-tight">
            {gym?.name}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
              ...
              <>
                ...
                <div className="absolute right-0 mt-3 w-72 sm:w-80 bg-white border-3 border-black rounded-xl shadow-[4px_4px_0px_0px_#000000] z-40 p-4 space-y-3">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {stats.activityFeed.length === 0 ? (
                      <p className="text-center py-6 text-xs font-bold text-slate-500 uppercase">
                        No recent updates
                      </p>
                    ) : (
          ...
          >
            {mobileMenuOpen ? (
              ...
              <svg
                className="w-4.5 h-4.5 text-black stroke-[3]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <div className="flex items-center gap-3 p-3 bg-brutal-bg/40 border-2 border-black rounded-lg">
              {gym?.logo_url ? (
                ...
                <img
                  src={gym.logo_url}
                  alt="Logo"
                  className="w-8 h-8 border border-black rounded object-cover bg-white"
                />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-black text-black text-xs truncate uppercase tracking-tight">
                  {gym?.name}
                </h4>
                <p className="text-slate-700 text-[9px] font-bold font-mono truncate">
      ...
      <aside className="hidden md:flex w-64 bg-white border-r-3 border-black flex-col shrink-0 min-h-screen">
        ...
        <div className="p-5 border-b-3 border-black bg-brutal-bg/40">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-black text-black text-sm truncate uppercase tracking-tight">
                {gym?.name}
              </h4>
              <p className="text-slate-700 text-[10px] font-bold font-mono truncate">
                {gym?.slug}.mygymsaas.com
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-40">
          <div className="space-y-1">
            ...
            <p className="text-slate-700 text-xs sm:text-sm font-bold">
              {activeTab === "overview" &&
                `Quick summary of gym performance and income status.`}
              {activeTab === "members" &&
                `View list of all gym members, expiry dates, and statuses.`}
              {activeTab === "payments" &&
                `Verify member fee payments (UPI transaction checks or cash approvals).`}
              {activeTab === "plans" &&
                `Create and manage membership plans and fee packages.`}
              {activeTab === "qr" &&
                `Display this QR at your counter desk so new members can join directly.`}
              {activeTab === "profile" &&
                `Edit gym contact details, address, and UPI payments receiver settings.`}
            </p>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              ...
              <div className="neo-card p-6 bg-white border-l-brutal-red shadow-[4px_4px_0px_0px_#000000] relative overflow-hidden group">
                ...
                <p className="text-3xl font-extrabold text-brutal-red mt-2">
                  {stats.expired}
                </p>
              </div>
              ...
              <div className="neo-card p-6 bg-white border-l-brutal-purple shadow-[4px_4px_0px_0px_#000000] relative overflow-hidden group">
                ...
                <p className="text-3xl font-extrabold text-brutal-purple mt-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="neo-card p-6 bg-white shadow-[4px_4px_0px_0px_#000000]">
                <h3 className="text-lg font-black uppercase text-black mb-6 border-b-3 border-black pb-2">
                  ...
                  <div className="flex-1 flex items-end gap-3 justify-around pt-4 pb-2 px-4 border-b-3 border-black relative bg-[#fafafa]">
                    ...
                    {newMembersChartData.map((d, idx) => {
                      const maxCount = Math.max(
                        ...newMembersChartData.map((item) => item.count),
                        5,
                      );
                      ...
                      return (
                        <div
                          key={idx}
                          className="flex-1 flex flex-col items-center group/bar z-10"
                        >
                  ...
                  <div className="flex-1 flex items-end gap-3 justify-around pt-4 pb-2 px-4 border-b-3 border-black relative bg-[#fafafa]">
                    ...
                    {revenueChartData.map((d, idx) => {
                      const maxRev = Math.max(
                        ...revenueChartData.map((item) => item.revenue),
                        10000,
                      );
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="flex justify-between items-center mb-3">
                  ...
                  <div className="text-right">
                    <span className="text-xs font-bold text-slate-650">
                      ₹{stats.revenueThisMonth} / ₹{stats.targetRevenue}
                    </span>
                    <span className="block text-lg font-black text-brutal-purple">
                ...
                <div className="w-full bg-[#f4f2ed] border-3 border-black rounded-lg h-7 overflow-hidden relative shadow-[2px_2px_0px_0px_#000000]">
                  ...
                  <span className="absolute inset-0 flex items-center justify-center text-xs font-extrabold uppercase text-black mix-blend-difference pointer-events-none">
                    {stats.targetProgress >= 100
                      ? "🎉 Goal Achieved!"
                      : `${100 - stats.targetProgress}% to target`}
                  </span>
                </div>
              </div>
              ...
              <div className="neo-card p-6 bg-white shadow-[4px_4px_0px_0px_#000000]">
                <h3 className="text-lg font-black uppercase text-black mb-4 border-b-3 border-black pb-2">
                  Plan Preferences Ratios
                </h3>

                {stats.planDistribution.length === 0 ? (
                  <p className="text-slate-650 text-xs font-bold">
                    No plan purchase history available yet.
                  </p>
                ) : (
                  <div className="space-y-3.5 pt-2">
                    {stats.planDistribution.map((dist, idx) => {
                      const totalMemberships = stats.planDistribution.reduce(
                        (acc, curr) => acc + curr.value,
                        0,
                      );
                      const ratio =
                        totalMemberships > 0
                          ? Math.round((dist.value / totalMemberships) * 100)
                          : 0;
                      const colors = [
                        "bg-brutal-cyan",
                        "bg-brutal-purple",
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="neo-card p-5 bg-white border-3 border-black shadow-[4px_4px_0px_0px_#000000]">
                <span className="block text-xs text-slate-500 font-black uppercase tracking-wider">
                  MoM Signups
                </span>
                <div className="flex items-baseline gap-2 mt-2">
                  ...
                  <span
                    className={`text-[10px] font-black border border-black px-1.5 py-0.5 rounded ${
                      stats.momGrowth >= 0
                ...
                <span className="block text-[10px] text-slate-650 font-bold mt-1.5 uppercase">
                  vs. {stats.signupsLastMonth} last month
                </span>
              </div>
              ...
              <div className="neo-card p-5 bg-white border-3 border-black shadow-[4px_4px_0px_0px_#000000]">
                ...
                <span className="block text-[10px] text-slate-650 font-bold mt-1.5 uppercase">
                  vs. ₹{stats.revenueLastMonth} last month
                </span>
              </div>

              {/* Expiry Risk Warning Box */}
              <div
                className={`neo-card p-5 border-3 border-black shadow-[4px_4px_0px_0px_#000000] flex flex-col justify-center ${
                  stats.expiringSoonCount > 0
                    ? "bg-brutal-orange/15 text-black"
                    : "bg-white text-black"
                }`}
              >
                <span className="block text-xs text-slate-500 font-black uppercase tracking-wider">
                  ...
                  <tbody className="divide-y-2 divide-black text-sm">
                    {filteredMembers.length === 0 ? (
                      ...
                      filteredMembers.map((member) => {
                        const expiryDate = member.expires_at
                        ...
                        const memberPlanName = memberMembership
                          ? plans.find((p) => p.id === memberMembership.plan_id)
                              ?.name || "Custom Plan"
                          : "No Plan";
                        return (
                          <tr
                          ...
                          >
                            <td
                              onClick={() =>
                                setShowMemberModal({ type: "profile", member })
                              }
                            ...
                            >
                              ...
                              <div className="text-slate-700 text-xs font-mono font-bold">
                                {member.phone}
                              </div>
                            </td>
                          ...
                          >
                            ...
                            <td className="p-4 sm:p-5 border-r-2 border-black/10">
                              <div className="flex items-center gap-2">
                                ...
                                {p.screenshot_url && (
                                  <button
                                    onClick={() =>
                                      setViewScreenshotUrl(p.screenshot_url)
                                    }
                            ...
                            <td className="p-4 sm:p-5 text-right">
                              {p.status === "pending" ? (
                                ...
                                <span className="text-slate-500 text-xs italic font-bold">
                                  Reviewed
                                </span>
                              )}
                            </td>
                ...
                >
                  ...
                  <div className="pt-2">
                    <div className="flex justify-between items-start gap-4 mb-4">
                      <h3 className="font-extrabold text-black text-lg uppercase tracking-tight truncate">
                        {p.name}
                      </h3>
                      <button
                    ...
                    <p className="text-slate-800 text-xs sm:text-sm font-semibold mb-6 line-clamp-2">
        ...
        {activeTab === "qr" && (
          <div className="max-w-2xl mx-auto space-y-6 text-center">
            <p className="text-slate-800 font-bold text-sm">
              Branded onboarding QR code. Display it at the counter for seamless
              membership access.
            </p>
            {gym && (
              <QRGenerator
            ...
            <p className="text-xs text-slate-500 font-bold italic mt-4">
              Tip: Print this QR in large high contrast format, laminate it, and
              place it at your billing desk.
            </p>
          </div>
        )}
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-[10px] text-slate-650 font-bold">
                    Payments will settle directly into this address.
                  </p>
                </div>
                  ...
                  {profileForm.logo_url && (
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      <img
                        ...
                        alt="Logo preview"
                        className="w-12 h-12 border-2 border-black rounded object-cover bg-white"
                      />
                      <button
                        type="button"
                        onClick={() =>
                ...
                <div>
                  ...
                  {profileForm.upi_qr_url && (
                    <div className="mt-3 flex items-center gap-2">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={profileForm.upi_qr_url}
                        alt="UPI QR preview"
                        className="w-12 h-12 border-2 border-black rounded object-contain bg-white p-0.5"
                      />
                      <button
                        type="button"
                        onClick={() =>
                ...
                {/* Section Header: Social & Video Embed */}
                <div className="md:col-span-2 border-t-2 border-black pt-5 mt-2">
                  <h3 className="font-extrabold text-sm uppercase text-black">Social Links & Video Promo</h3>
                  <p className="text-slate-650 text-[10px] font-bold">Configure media links to showcase on your gym's custom landing page.</p>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. https://instagram.com/goldsgym"
                    value={profileForm.instagram_url || ""}
                    onChange={(e) =>
                      setProfileForm({ ...profileForm, instagram_url: e.target.value })
                    }
                    className="neo-input block w-full text-sm font-mono text-brutal-purple"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setProfileForm({ ...profileForm, youtube_url: e.target.value })
                    }
                    className="neo-input block w-full text-sm font-mono text-red-600"
                  />
                </div>

                {/* Section Header: Photo Gallery */}
                <div className="md:col-span-2 border-t-2 border-black pt-5 mt-2">
                  <h3 className="font-extrabold text-sm uppercase text-black">Gym Photo Gallery</h3>
                  <p className="text-slate-650 text-[10px] font-bold">Upload up to 3 high-quality photos showing off your gym interior, equipment, or workout floor.</p>
                </div>

                {/* Gallery Photo 1 */}
                <div className="p-4 border-2 border-black bg-[#fafafa] rounded-xl shadow-[2.5px_2.5px_0px_0px_#000000]">
                  <label className="block text-xs font-black text-slate-800 uppercase tracking-wider mb-2">
                    Gallery Photo 1
                  </label>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleFileUpload(e, "gallery_photo1")}
                    className="block w-full text-xs text-slate-700 file:mr-2 file:py-1 file:px-2 file:rounded file:border-2 file:border-black file:text-[10px] file:font-black file:bg-white file:text-black file:cursor-pointer hover:file:bg-slate-100 transition"
                  />
                  {profileForm.gallery_photo1 && (
                    <div className="mt-3 flex items-center gap-2">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={profileForm.gallery_photo1}
                        alt="Gallery 1 preview"
                        className="w-16 h-10 border-2 border-black rounded object-cover bg-white animate-fade-in"
                      />
                      <button
                        type="button"
                        onClick={() =>
                ...
                <div className="p-4 border-2 border-black bg-[#fafafa] rounded-xl shadow-[2.5px_2.5px_0px_0px_#000000]">
                  ...
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleFileUpload(e, "gallery_photo2")}
                    className="block w-full text-xs text-slate-700 file:mr-2 file:py-1 file:px-2 file:rounded file:border-2 file:border-black file:text-[10px] file:font-black file:bg-white file:text-black file:cursor-pointer hover:file:bg-slate-100 transition"
                  />
                  {profileForm.gallery_photo2 && (
                    <div className="mt-3 flex items-center gap-2">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={profileForm.gallery_photo2}
                ...
                <div className="p-4 border-2 border-black bg-[#fafafa] rounded-xl shadow-[2.5px_2.5px_0px_0px_#000000]">
                  ...
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleFileUpload(e, "gallery_photo3")}
                    className="block w-full text-xs text-slate-700 file:mr-2 file:py-1 file:px-2 file:rounded file:border-2 file:border-black file:text-[10px] file:font-black file:bg-white file:text-black file:cursor-pointer hover:file:bg-slate-100 transition"
                  />
                  {profileForm.gallery_photo3 && (
                    <div className="mt-3 flex items-center gap-2">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={profileForm.gallery_photo3}
      ...
      {showPlanModal && (
        <div className="fixed inset-0 z-50 bg-black/60 flex justify-center items-center p-4">
          <div className="neo-card w-full max-w-md p-8 bg-white border-3 border-black shadow-[8px_8px_0px_0px_#000000] relative">
            ...
            <h3 className="text-xl font-extrabold uppercase mb-6 border-b-3 border-black pb-2">
              Create Custom Plan
            </h3>
            ...
            {showMemberModal.type === "profile" ? (
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-4 border-b-3 border-black pb-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {/* Status Badge */}
                    <span
                      className={`inline-flex items-center mt-2 px-2.5 py-0.5 rounded border-2 border-black text-[10px] font-black uppercase ${
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-white border-2 border-black rounded-lg shadow-[2.5px_2.5px_0px_0px_#000000]">
                    <span className="block text-[10px] text-slate-500 font-black uppercase">
                      First Joined
                    </span>
                    <span className="block text-sm font-extrabold text-black font-mono mt-0.5">
                      {new Date(
                        showMemberModal.member.joined_at,
                  ...
                  <div className="p-3 bg-white border-2 border-black rounded-lg shadow-[2.5px_2.5px_0px_0px_#000000]">
                    ...
                    <span className="block text-sm font-extrabold text-black font-mono mt-0.5">
                      {showMemberModal.member.expires_at
                        ? new Date(
                            showMemberModal.member.expires_at,
                          ).toLocaleDateString("en-IN", {
                            day: "numeric",
                ...
                <div className="space-y-3 pt-2">
                  ...
                  {payments.filter(
                    (p) => p.member_id === showMemberModal.member.id,
                  ).length === 0 ? (
                    <p className="text-xs font-bold text-slate-700 bg-brutal-bg p-3 border-2 border-black rounded">
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {payments
                        ...
                        .map((p) => (
                          <div
                            key={p.id}
                            className="flex justify-between items-center p-3 border-2 border-black rounded bg-[#fafafa]"
                          >
                            <div>
                            ...
                            <div className="text-right">
                              <span
                                className={`inline-flex items-center px-2 py-0.5 rounded border border-black text-[9px] font-black uppercase ${
                                  p.status === "approved"
                                    ? "bg-brutal-green/20 text-emerald-800"
                                    : p.status === "rejected"
                                      ? "bg-brutal-red/20 text-red-800"
              ...
              <>
                ...
                <p className="text-xs font-bold text-slate-700 mb-6">
                  Change membership options for{" "}
                  <span className="text-brutal-orange font-black uppercase">
                    {showMemberModal.member.name}
                  </span>
                </p>
      ...
      {viewScreenshotUrl && (
        <div className="fixed inset-0 z-50 bg-black/90 flex justify-center items-center p-4">
          <div className="relative max-w-2xl w-full flex flex-col items-center">
            ...
            <p className="mt-4 text-xs font-bold text-white uppercase tracking-wider bg-black/50 px-3 py-1 rounded">
              Transaction Slip Proof
            </p>
          </div>
        </div>
      )}
